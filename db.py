...
def save_call_log(
    ...
    key = os.environ.get("SUPABASE_KEY", "")
    if not url or not key:
        logger.info(f"Supabase not configured. Local log duration={duration}s phone={mask_phone(phone)}")
    ...
    def _try_insert(data: dict, label: str) -> dict:
        for attempt in range(_MAX_RETRIES):
            try:
                ...
                logger.info(f"Saved call log for {mask_phone(phone)} ({label})")
                ...
                if _is_retryable(err) and attempt < _MAX_RETRIES - 1:
                    ...
                    logger.warning(
                        f"Transient call-log save error (attempt {attempt+1}), retrying in {delay}s"
                    )
                    time.sleep(delay)
                    continue
                logger.error(f"Failed to save call log ({label}): {type(e).__name__}")
